...
public:
    ...
    //                 (which areused to produce different digests).
    ...
    SBF(int bit_mapping, int HASH_family, int HASH_number, int AREA_number, std::string salt_path)
    {
        ...
        // Argumnet validation
        if(bit_mapping<=0 || bit_mapping > MAX_BIT_MAPPING) throw std::invalid_argument("Invalid bit mapping. It must be in [1,32]");
        if(AREA_number<=0 || AREA_number > MAX_AREA_NUMBER) throw std::invalid_argument("Invalid number of areas. The number must be in [1,65535]");
        if(HASH_number<=0 || HASH_number > MAX_HASH_NUMBER) throw std::invalid_argument("Invalid number of hash runs. It must be in [1,4096]");
